<?php
/* @var $this CccfCustomFieldController */
/* @var $model BaseCccfCustomField */

$this->breadcrumbs=array(
	'Base Cccf Custom Fields'=>array('index'),
	$model->title,
);

$this->menu=array(
	array('label'=>'List BaseCccfCustomField', 'url'=>array('index')),
	array('label'=>'Create BaseCccfCustomField', 'url'=>array('create')),
	array('label'=>'Update BaseCccfCustomField', 'url'=>array('update', 'id'=>$model->id)),
	array('label'=>'Delete BaseCccfCustomField', 'url'=>'#', 'linkOptions'=>array('submit'=>array('delete','id'=>$model->id),'confirm'=>'Are you sure you want to delete this item?')),
	array('label'=>'Manage BaseCccfCustomField', 'url'=>array('admin')),
);
?>

<h1>View BaseCccfCustomField #<?php echo $model->id; ?></h1>

<?php $this->widget('zii.widgets.CDetailView', array(
	'data'=>$model,
	'attributes'=>array(
		'id',
		'varname',
		'title',
		'field_type',
		'field_size',
		'field_size_min',
		'required',
		'match',
		'range',
		'error_message',
		'other_validator',
		'default',
		'widget',
		'widgetparams',
		'position',
		'visible',
	),
)); ?>
